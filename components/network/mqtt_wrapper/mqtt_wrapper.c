...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA: {
        ...
        mqtt_full_cmd_t cmd_ota = {0};
        if (json_parse_ota_command(json_string, &cmd_ota.ota) == ESP_OK) {
            if (cmd_ota.ota.has_url) {
                cmd_ota.type = CMD_TYPE_OTA;
                if (mqtt_cmd_queue != NULL) xQueueSend(mqtt_cmd_queue, &cmd_ota, 0);
            }
        }
...
void task_cmd_manager(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (xQueueReceive(mqtt_cmd_queue, &cmd, portMAX_DELAY) == pdTRUE) {
            ...
            if (cmd.type == CMD_TYPE_OTA) {
                ESP_LOGI("CMD_TASK", "OTA command received. URL: %s", cmd.ota.url);
                
                if (ota_service_start_update(cmd.ota.url)) {
                    esp_mqtt_client_publish(global_client, MQTT_STATUS_TOPIC, "ota: started", 0, 1, 1);
                } else {
                    esp_mqtt_client_publish(global_client, MQTT_STATUS_TOPIC, "ota: error (already running or bad URL)", 0, 1, 1);
                }
            }
        }
    }
