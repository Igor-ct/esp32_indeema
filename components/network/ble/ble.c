...
static int gatt_svr_chr_access_ota(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg);
...
static const ble_uuid128_t gatt_svr_svc_ota_uuid =
    BLE_UUID128_INIT(0x00, 0x00, 0x19, 0xed, 0x82, 0xae, 0xed, 0x21, 0x4c, 0x9d, 0x41, 0x45, 0x00, 0x10, 0x22, 0x8e);

static const ble_uuid128_t gatt_svr_chr_ota_cmd_uuid =
    BLE_UUID128_INIT(0x00, 0x00, 0x19, 0xed, 0x82, 0xae, 0xed, 0x21, 0x4c, 0x9d, 0x41, 0x45, 0x01, 0x10, 0x22, 0x8e);
...
static const struct ble_gatt_svc_def gatt_svr_svcs[] = {
    ...
    {
        .type = BLE_GATT_SVC_TYPE_PRIMARY,
        .uuid = (ble_uuid_t*)&gatt_svr_svc_ota_uuid,
        .characteristics = (struct ble_gatt_chr_def[]) { 
            { 
                .uuid = (ble_uuid_t*)&gatt_svr_chr_ota_cmd_uuid, 
                ...
                .descriptors = (struct ble_gatt_dsc_def[]) {
                    {
                        ...
                        .att_flags = BLE_ATT_F_READ,
                        .access_cb = gatt_svr_dsc_access,
                        .arg = (void *)"OTA Update (Write URL String)"
                    },
                    {0}
                }
            }, 
...
static int gatt_svr_chr_access_ota(uint16_t conn_handle, uint16_t attr_handle, 
                                   struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
        ...
        if (len == 0 || len > 255) {
            ESP_LOGE("BLE_OTA", "Invalid URL length: %d", len);
            return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;
        }

        char url_buf[256] = {0};
        ble_hs_mbuf_to_flat(ctxt->om, url_buf, len, NULL);

        ESP_LOGI("BLE_OTA", "Received OTA URL via BLE: %s", url_buf);

        if (ota_service_start_update(url_buf)) {
            ESP_LOGI("BLE_OTA", "OTA started successfully.");
        } else {
            ESP_LOGE("BLE_OTA", "Failed to start OTA (already running or invalid URL).");
        }
