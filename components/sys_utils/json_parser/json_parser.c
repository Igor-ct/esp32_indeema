...
esp_err_t json_parse_ota_command(const char *json_string, parsed_ota_cmd_t *out_cmd) {
    ...
    cJSON *ota_url = cJSON_GetObjectItem(root, "ota");
    if (cJSON_IsString(ota_url) && (ota_url->valuestring != NULL)) {
        out_cmd->has_url = true;
        
        strncpy(out_cmd->url, ota_url->valuestring, sizeof(out_cmd->url) - 1);
        out_cmd->url[sizeof(out_cmd->url) - 1] = '\0';
    }
